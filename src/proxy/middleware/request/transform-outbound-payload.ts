...
export const transformOutboundPayload: RequestPreprocessor = async (req) => {
  ...
  if (req.inboundApi === "openai" && req.outboundApi === "gepetto") {
    req.body = openAiToGepetto(req);
    req.log.debug("Transformed OpenAI-to-Gepetto request");
    return;
  }
...
function openAiToGepetto(req: Request) {
  // I can't figure out the request format for whatever the hell this endpoint
  // is supposed to be. It seems to ignore most of the request body except for
  // the prompt.

  // Until I can confirm otherwise we will just validate and send the request
  // body verbatim.
  ...
  if (!result.success) {
    req.log.error(
      { issues: result.error.issues, body: req.body },
      "Invalid OpenAI-to-Gepetto request"
    );
    throw result.error;
  }
